...
function initialize(){
	...
	// returns the layout height of the element as an integer 
...
function drawGraph1(){
	...
	for(var i = 0; i <= data.max; i += 5){
		...
		// moveTo :시작지점 
		ctx.moveTo(50, y);
		//새 점을 추가하고 캔버스의 마지막 지정된 점에서 해당 점까지 선을 만듭니다
		ctx.lineTo(graph1.width - 20, y);
		ctx.fillText(data.max - i, 40, y);
	}
	//획에 사용되는 색상, 그라디언트 또는 패턴을 설정하거나 반환
	// context .strokeStyle = 색상 | 그라데이션 | 패턴 ;
